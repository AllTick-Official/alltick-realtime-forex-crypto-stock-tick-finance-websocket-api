> [English](./rest_get_suspension_query.md) | [中文](./rest_get_suspension_query_cn.md)
...
| ------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Free          | Only 1 request per minute | 1、One request per second.2、/batch-kline needs 10-second intervals.3、Total of 10 requests per minute (every 6 seconds).4、Max 1000 daily requests; excess resets at midnight.                           |
| Basic         | Only 1 request per minute | 1、One request per second.2、/batch-kline: 1 request every 3 seconds.3、Total of 60 requests per minute (1 request per second).4、Max 86400 daily requests; excess resets at midnight.                    |
| Premium       | Only 1 request per minute | 1、Combined interfaces: 10 requests/second.2、/batch-kline: 1 request/2 seconds.3、Total: 600 requests/minute (10/second).4、Daily limit: 864,000 requests; reset daily at midnight if exceeded.          |
| Professional  | Only 1 request per minute | 1、Combined interfaces: 20 requests/second.2、/batch-kline: 1 request/second interval.3、Total: 1200 requests/minute (20/second).4、Daily limit: 1,728,000 requests; reset daily at midnight if exceeded. |
| All HK Stocks | Only 1 request per minute | 1、Combined interfaces: 20 requests/second.2、/batch-kline: 1 request/second interval.3、Total: 1200 requests/minute (20/second).4、Daily limit: 1,728,000 requests; reset daily at midnight if exceeded. |
| All CN Stocks | Only 1 request per minute | 1、Combined interfaces: 20 requests/second.2、/batch-kline: 1 request/second interval.3、Total: 1200 requests/minute (20/second).4、Daily limit: 1,728,000 requests; reset daily at midnight if exceeded. |
| All US Stocks | Only 1 request per minute | 1、Combined interfaces: 20 requests/second.2、/batch-kline: 1 request/second interval.3、Total: 1200 requests/minute (20/second).4、Daily limit: 1,728,000 requests; reset daily at midnight if exceeded. |
...
- Base Path: /v1/trading-status/sse
- Full URL: <https://quote.alltick.co/v1/trading-status/sse>

#### 2、Query Suspension/Resumption Information of the New York Stock Exchange (NYSE):

- Base Path: /v1/trading-status/nyse
- Full URL: <https://quote.alltick.co/v1/trading-status/nyse>

#### 3、Query Suspension/Resumption Information of the Nasdaq Stock Exchange:

- Base Path: /v1/trading-status/nasdaq
- Full URL: <https://quote.alltick.co/v1/trading-status/nasdaq>
...
- **URL**: `/v1/trading-status/sse`
- **Method**: `GET`
- **Description**: Retrieve all suspension and resumption information from the Shanghai Stock Exchange (SSE).

#### Example Request

curl -X GET "<https://quote.alltick.co/v1/trading-status/sse/{page}/{size}>"

### Request Parameters

| Field     | Position | Type   | Required | Description             |
| --------- | :------- | ------ | -------- | ----------------------- |
| X-API-Key | header   | string | Yes      | User subscription token |
| page      | query    | int    | Yes      | Page number             |
| size      | query    | int    | Yes      | Page size               |
...
- **URL**: `/v1/trading-status/nasdaq`
- **Method**: `GET`
- **Description**  Retrieve all suspension and resumption information from the NASDAQ Stock Exchange.

#### Example Request

curl -X GET "<https://quote.alltick.co/v1/trading-status/nasdaq/{page}/{size}>"
...
| --------- | :------- | ------ | -------- | ----------------------- |
| X-API-Key | header   | string | Yes      | User subscription token |
| page      | path    | int    | Yes      | Page number             |
| size      | path    | int    | Yes      | Page size               |
